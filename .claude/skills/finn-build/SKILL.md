...
Using the Linear connector, list issues on team `LIS` that meet every
condition:
...
- Fetch the latest default branch from `origin` and create or resume a branch
  named `LIS-NNN-short-slug`, using the issue's real identifier.
...
- `Closes LIS-NNN`, using the real Linear issue identifier
